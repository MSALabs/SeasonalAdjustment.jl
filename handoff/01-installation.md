...
During the alpha, neither this package nor `TSAnalytics.jl` is in Julia's
General registry, so both are installed together from their repositories:
...
Both must go in **one** call. Two separate `Pkg.add` calls fail in either order,
because whichever runs first triggers a dependency resolution that needs the
other package already present.

After registration this becomes `Pkg.add("SeasonalAdjustment")`.

## Nothing else to install

Whichever way you installed it, **X-13 itself came with the package.** There is
no second download, no `PATH` entry, no system-wide install.
